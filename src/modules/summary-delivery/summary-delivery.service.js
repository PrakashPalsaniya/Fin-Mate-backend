const SummaryDelivery = require("./summary-delivery.model.js");
const { normalizeUserSettings } = require("../settings/settings.utils.js");
const { normalizeSummaryRange } = require("../ai/ai.service.js");
const { sendEmail } = require("../../shared/services/email.service.js");
const { sendMessage } = require("../telegram/services/telegramApi.js");
const {
    buildSummaryEmailContent,
    buildSummarySnapshot,
    buildSummaryTelegramContent,
} = require("./summary-digest.service.js");
const { getOrCreateSummaryPayload } = require("../ai/ai-cache.service.js");
const {
    getWeekStartDateKey,
    getZonedDateParts,
    isLocalTimeAtOrAfter,
} = require("../../shared/utils/timezone.js");
const {
    getRawSummaryDeliveryError,
    sanitizeSummaryDeliveryError,
} = require("./summary-delivery.error.js");
...
const deliverSummary = async ({
    ...
    const claimResults = await Promise.all(channels.map(async (channel) => ({
        channel,
        claimResult: await claimSummaryDelivery({
            userId: user._id,
            frequency: normalizedFrequency,
            channel,
    ...
    try {
        ...
        const results = await Promise.all(claimResults.map(async ({ channel, claimResult }) => {
            if (!claimResult.claimed) return mapUnclaimedSummaryChannelResult({ channel, claimResult });
            await markSummaryDeliveryFailed({ record: claimResult.record, error });
            return { channel, status: "failed", error: sanitizeSummaryDeliveryError({ channel, error }) };
        }));

        return { frequency: normalizedFrequency, deliveryKey: effectiveDeliveryKey, summary: null, channels: results };
    }
    ...
    const results = await Promise.all(claimResults.map(async ({ channel, claimResult }) => {
        ...
        try {
            const sendResult = await sendSummaryToChannel({ channel, user, frequency: normalizedFrequency, summary, timeZone });
            await markSummaryDeliverySent({ record: claimResult.record, summary, providerMessageId: sendResult.providerMessageId });
