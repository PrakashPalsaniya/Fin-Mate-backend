...
const { getZonedDateParts } = require("../../shared/utils/timezone.js");
const {
  getGeminiTextParts,
  getGeminiUrl,
  isGeminiQuotaError,
...
const generateAISummary = async (req, res) => {
  try {
    ...
    const [
      ...
      Income.aggregate([{ $match: { userId: userObjectId, date: { $gte: thirtyDaysAgo } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      Expense.aggregate([{ $match: { userId: userObjectId, date: { $gte: thirtyDaysAgo } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      Income.aggregate([{ $match: { userId: userObjectId, date: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      Expense.aggregate([{ $match: { userId: userObjectId, date: { $gte: sixtyDaysAgo, $lt: thirtyDaysAgo } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      Expense.aggregate([{ $match: { userId: userObjectId, date: { $gte: thirtyDaysAgo } } }, { $group: { _id: "$category", total: { $sum: "$amount" } } }, { $sort: { total: -1 } }]),
      Income.aggregate([{ $match: { userId: userObjectId, date: { $gte: thirtyDaysAgo } } }, { $group: { _id: "$category", total: { $sum: "$amount" } } }, { $sort: { total: -1 } }]),
      Expense.find({ userId, date: { $gte: thirtyDaysAgo } }).sort({ date: -1 }).limit(10),
      Income.find({ userId, date: { $gte: thirtyDaysAgo } }).sort({ date: -1 }).limit(10)
    ]);
    ...
    const previousIncome = previousMonthIncomeResult[0]?.total || 0;
    const previousExpenses = previousMonthExpenseResult[0]?.total || 0;
    
    const incomeChange = previousIncome > 0 ? ((totalIncome - previousIncome) / previousIncome * 100).toFixed(1) : null;
    const expenseChange = previousExpenses > 0 ? ((totalExpenses - previousExpenses) / previousExpenses * 100).toFixed(1) : null;

    const dailyExpenseRate = (totalExpenses / 30).toFixed(0);
